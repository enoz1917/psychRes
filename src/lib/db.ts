  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.log(`Questionnaires table exists check: ${tableExists}`);
        
        // If the table exists, check if it has the new columns
        if (tableExists) {
          console.log('Checking if table needs to be updated with new columns...');
          ...
          console.log('Column check results:', columnCheck[0]);
          
          // If any new column is missing, add them
          if (!columnCheck[0].has_section2_38 || !columnCheck[0].has_section3_15 || !columnCheck[0].has_section4_30) {
            console.log('Adding missing columns to questionnaires table...');
            ...
            console.log('All missing columns have been added to the questionnaires table');
          } else {
            console.log('All required columns already exist in the questionnaires table');
          }
        } else {
          // Create new questionnaires table with individual columns
          console.log('Questionnaires table does not exist. Creating it now...');
          await sql`
          ...
          `;
          console.log('Questionnaires table created successfully with all required columns');
        }
      } catch (questionnairesError) {
        console.error('Error creating/updating questionnaires table:', questionnairesError);
        console.error('Error details:', questionnairesError instanceof Error ? questionnairesError.stack : 'No stack trace');
...
export async function saveQuestionnaire(
  ...
  console.log('Initializing database before saving questionnaire...');
  const initResult = await initDatabase();
  if (!initResult) {
    console.error('Database initialization failed when saving questionnaire');
    return { 
      id: null, 
  ...
  console.log('Question data has', Object.keys(questionData).length, 'fields');
  
  try {
    // First check if the questionnaires table exists
    ...
    if (!tableCheck[0]?.exists) {
      console.error('questionnaires table does not exist in the database!');
      throw new Error('questionnaires table not found in the database');
    }
    ...
    if (existingRecord && existingRecord.length > 0) {
      ...
      console.log(`Insert query prepared with ${insertValues.length} values`);
      ...
      try {
        ...
        console.error('Query details:', { 
          ...
          firstFewColumns: columnNames.slice(0, 5),
          lastFewColumns: columnNames.slice(-5)
        });
        throw insertError;
      }
    }
